...
@st.fragment
def render_task_lists(player_id: int, war_mode_active: bool) -> None:
    tasks = db.list_checklist_tasks(player_id)
                ...
                with st.container(border=True):
                    ...
                    with cols[0]:
                        if st.checkbox("Done", key=f"done_{task['id']}"):
                            db.set_task_done(player_id, task["id"], True, int(time.time()))
                    ...
                    with cols[3]:
                        if st.button("Delete", key=f"delete_{task['id']}"):
                            db.delete_checklist_task(player_id, task["id"])
                            st.rerun(scope="fragment")
            ...
            for task in done_tasks:
                ...
                with cols[0]:
                    if st.checkbox("Done", value=True, key=f"undo_{task['id']}"):
                        ...
                        db.set_task_done(player_id, task["id"], False, task["last_completed_at"])
                        st.rerun(scope="fragment")
                with cols[1]:
                    st.markdown(f"~~{task['title']}~~")


render_task_lists(player.player_id, war_mode_active)
