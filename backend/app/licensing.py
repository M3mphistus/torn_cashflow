...
GROUP_DISCOUNT_TIERS = [
    (1, 0.00), (10, 0.10), (25, 0.20), (50, 0.30), (100, 0.40),
]
...
def is_admin(player: CurrentPlayer) -> bool:
    return player.player_id == settings.dev_torn_player_id
...
def get_premium_status(player: CurrentPlayer) -> PremiumStatus:
...
def start_free_trial(player: CurrentPlayer) -> TrialResult:
...
def _qualifying_entries(player: CurrentPlayer, lookback_days: int) -> tuple[list[dict], list[dict], set[str]]:
    now_ts = int(time.time())
    ...
    dev_id = settings.dev_torn_player_id
    qualifying = [
        e for e in entries
...
def _xanax_qty(entry: dict) -> int:
    item_id = settings.xanax_item_id
    return sum(i["qty"] or 0 for i in entry["items"] if i["id"] == item_id)


def scan_and_activate_payment(player: CurrentPlayer, lookback_days: int = 7) -> ScanResult:
    now_ts = int(time.time())
    ...
    if total_xanax <= 0:
        return ScanResult(
            credited_count=0, weeks_added=0, new_premium_until=None, already_credited_count=len(all_entries)
        )
    ...
    db.upsert_license(
        "individual", player.player_id, new_until, origin="payment", last_payment_torn_log_id=latest_log_id
    )
...
def get_faction_requirement_preview(player: CurrentPlayer) -> FactionRequirementPreview | None:
...
def scan_and_activate_group_payment(player: CurrentPlayer, lookback_days: int = 7) -> GroupScanResult:
    if not player.faction_id:
